...
class PaymentServices {
  ...
  static Future<({List<TransactionsModel>? success, String? error})> getTransactions(
      String id) async {
    try {
      final response = await dio.get("${baseUrl}couriers/$id/transactions");

      if (response.data["status"] == "success") {
        final data =  (response.data["data"] as List).map((e) => TransactionsModel.fromMap(e)).toList();
